  ...
  return (
    <div className="relative" style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
        {/* علامة الطرق الآمنة */}
        <div 
          className={`px-4 py-3 rounded-lg shadow-lg cursor-pointer transition-all flex items-center gap-2 min-w-[200px] ${
            weatherSafetyData?.safeRoutes && Array.isArray(weatherSafetyData.safeRoutes) && weatherSafetyData.safeRoutes.length > 0
              ? 'bg-green-500 text-white hover:bg-green-600'
              : weatherSafetyData === null || weatherSafetyData === undefined
              ? 'bg-yellow-300 text-yellow-800 opacity-75'
          ...
          title={
            weatherSafetyData === null || weatherSafetyData === undefined
              ? 'جاري تحميل البيانات...'
              : weatherSafetyData?.safeRoutes && Array.isArray(weatherSafetyData.safeRoutes) && weatherSafetyData.safeRoutes.length > 0
              ? `${weatherSafetyData.safeRoutes.length} طريق آمن لجميع أحوال الطقس`
              : 'لا توجد طرق آمنة حالياً'
          }
        >
          ...
          <div className="flex-1">
            <div className="font-bold text-sm">طرق آمنة</div>
            <div className="text-xs opacity-90">
              {weatherSafetyData === null || weatherSafetyData === undefined
                ? 'جاري التحميل...'
                : weatherSafetyData?.safeRoutes && Array.isArray(weatherSafetyData.safeRoutes) && weatherSafetyData.safeRoutes.length > 0
                ? `${weatherSafetyData.safeRoutes.length} طريق`
                : 'لا توجد طرق آمنة'}
            </div>
          </div>
          <div className="text-lg font-bold">
            {weatherSafetyData === null || weatherSafetyData === undefined
              ? '...'
              : weatherSafetyData?.safeRoutes && Array.isArray(weatherSafetyData.safeRoutes)
              ? weatherSafetyData.safeRoutes.length
              : '0'}
          </div>
        </div>
        ...
        <div 
          className={`px-4 py-3 rounded-lg shadow-lg cursor-pointer transition-all flex items-center gap-2 min-w-[200px] ${
            weatherSafetyData?.unsafeRoutes && Array.isArray(weatherSafetyData.unsafeRoutes) && weatherSafetyData.unsafeRoutes.length > 0
          ...
          title={
            weatherSafetyData === null || weatherSafetyData === undefined
              ? 'جاري تحميل البيانات...'
              : weatherSafetyData?.unsafeRoutes && Array.isArray(weatherSafetyData.unsafeRoutes) && weatherSafetyData.unsafeRoutes.length > 0
              ? `${weatherSafetyData.unsafeRoutes.length} طريق غير آمن لبعض أحوال الطقس`
              : 'لا توجد طرق غير آمنة حالياً'
          }
        >
          <div className="text-2xl">
            {weatherSafetyData === null || weatherSafetyData === undefined ? '⏳' : '⚠️'}
          </div>
          <div className="flex-1">
            <div className="font-bold text-sm">طرق غير آمنة</div>
            <div className="text-xs opacity-90">
              {weatherSafetyData === null || weatherSafetyData === undefined
                ? 'جاري التحميل...'
                : weatherSafetyData?.unsafeRoutes && Array.isArray(weatherSafetyData.unsafeRoutes) && weatherSafetyData.unsafeRoutes.length > 0
                ? `${weatherSafetyData.unsafeRoutes.length} طريق`
                : 'لا توجد طرق غير آمنة'}
            </div>
          </div>
          <div className="text-lg font-bold">
            {weatherSafetyData === null || weatherSafetyData === undefined
              ? '...'
              : weatherSafetyData?.unsafeRoutes && Array.isArray(weatherSafetyData.unsafeRoutes)
              ? weatherSafetyData.unsafeRoutes.length
              : '0'}
          </div>
        </div>
      </div>
